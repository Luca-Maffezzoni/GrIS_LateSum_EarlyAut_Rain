...
output_summer = os.path.join(
    ROOT, "..", "..", "..",
    "Data", "PROCESSED", "MET_GLACIO_PARAMETERS", "SITE_A",
    "runoff_summer_2013.txt"
)

output_autumn = os.path.join(
    ROOT, "..", "..", "..",
    "Data", "PROCESSED", "MET_GLACIO_PARAMETERS", "SITE_A",
    "runoff_autumn_2013.txt"
)
...
for j in range(0, 92):  # 92 giorni
    summer_file = os.path.join(
        ...
        "sum_RU_MARv3.9.2_NCEP1-20km_2013.nc"
    )

    fileobj1 = Dataset(summer_file)
    data_runoff = fileobj1.variables["RU"][j, :, :]

    ma_data_runoff = MA.masked_where(((mask < 219) | (mask > 220)), data_runoff)
    setattr(ma_data_runoff, "fill_value", float("NaN"))
    ma_data_masked_runoff = MA.filled(ma_data_runoff)

    ma_data_2_runoff = MA.masked_where(N.isnan(mask2), ma_data_masked_runoff)
    setattr(ma_data_2_runoff, "fill_value", float("NaN"))
    ma_data_masked_2_runoff = MA.filled(ma_data_2_runoff)

    mean_value = N.nanmean(ma_data_masked_2_runoff)
...
for i in range(0, 30):  # 30 giorni
    autumn_file = os.path.join(
        ...
        "autumn_RU_MARv3.9.2_NCEP1-20km_2013.nc"
    )

    fileobj2 = Dataset(autumn_file)
    data = fileobj2.variables["RU"][i, :, :]
