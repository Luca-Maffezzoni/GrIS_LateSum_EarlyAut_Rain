...
# variables that are Short Wave Downward (SWD), Long Wave Downard (LWD), Long Wave Upward (LWU), Sensible Heat Flux (SHF) and Latent Heat Flux (LHF) 
# over Greenland. The original MAR model provides yearly outputs from **1958 to 2016**.
#
...
for q in ["SWD","LWD","LWU","SHF","LHF"]:
    ...
    for x in files:
        ...
        # Extract time slices corresponding to the summer period
        time=np.array([],dtype="f")
        for w in range(151,243):
            p=fileobj.variables["TIME"][w]
            time=np.append(time,p)
        # Extract variable data
        z=fileobj.variables[q][151,:,:]
        for m in range(152,243):
            shf=fileobj.variables[q][m,:,:]
            z=np.concatenate((z,shf))
        z=np.reshape(z,(92,135,73))
        ...
        file_out.title = f"parameter_{q}_summer_{year}"
        file_out.close()